...
#include <wiringPi.h>  // 引入wiringOP库
...
// LED控制相关全局变量（修改为CH10控制，适配282和1722）
const int LED_PIN = 10;       // wPi编号10（对应GPIOA4）
bool led_state = false;       // 当前LED状态（false：灭，true：亮）
const int CH10_LOW = 282;     // CH10低状态值（LED关）
const int CH10_HIGH = 1722;   // CH10高状态值（LED开）
const int CH10_TOLERANCE = 5; // 容差范围（值稳定，缩小至±5）
int prev_ch10_state = -1;     // 上一次CH10状态（-1:未初始化, 0:低, 1:高）
int ch10_stable_count = 0;    // 连续稳定计数（去抖动）
const int REQUIRED_STABLE_COUNT = 3; // 需要连续3次稳定才确认状态
...
void printCurrentState() {
    ...
    ROS_INFO("[System State] Car mode: %s | Manual: %s | LED: %s", 
...
void printRemoteInfo() {
    ...
    // 更新为CH10状态信息（适配282和1722）
    uint16_t ch10_raw = latest_sbus_data.rawChannels[9];  // CH10对应索引9
    std::string ch10_state;
    if (abs(ch10_raw - CH10_HIGH) <= CH10_TOLERANCE) {
        ch10_state = "1722 (LED ON)";
    } else if (abs(ch10_raw - CH10_LOW) <= CH10_TOLERANCE) {
        ch10_state = "282 (LED OFF)";
    } else {
        ch10_state = "Unknown";
    }
    ...
    ROS_INFO("CH10(LED Control): Raw=%d | State=%s", ch10_raw, ch10_state.c_str());  // 显示CH10信息
    ...
    // 新增：打印所有SBUS通道原始值，用于排查通道映射
    ROS_INFO("All SBUS channels (CH1-CH16 Raw values):");
    for (int i = 0; i < 16; i++) {
        ROS_INFO("  CH%d: Raw=%d", i + 1, latest_sbus_data.rawChannels[i]);
    }
    ROS_INFO("--------------------------------------------");
}
...
void sbusCallback(const sbus_serial::Sbus::ConstPtr& msg) {
    ...
    // 解析CH10通道（索引9），控制LED（增加去抖动）
    uint16_t ch10_raw = msg->rawChannels[9];  // 获取CH10原始值
    int current_ch10_state = -1;

    // 判断当前CH10状态（高/低）
    if (abs(ch10_raw - CH10_HIGH) <= CH10_TOLERANCE) {
        current_ch10_state = 1;  // 高状态（1722，LED开）
    } else if (abs(ch10_raw - CH10_LOW) <= CH10_TOLERANCE) {
        current_ch10_state = 0;  // 低状态（282，LED关）
    }

    // 去抖动逻辑：连续REQUIRED_STABLE_COUNT次检测到同一状态才确认
    if (current_ch10_state != -1) {
        if (current_ch10_state == prev_ch10_state) {
            ...
            if (ch10_stable_count >= REQUIRED_STABLE_COUNT) {
                // 连续稳定次数达标，执行状态切换
                if (current_ch10_state == 1) {  // 切换到高状态：开灯
                    led_state = true;
                    digitalWrite(LED_PIN, HIGH);
                    ROS_INFO("LED turned ON (CH10 high: %d)", ch10_raw);
                } else {  // 切换到低状态：关灯
                    led_state = false;
                    digitalWrite(LED_PIN, LOW);
                    ROS_INFO("LED turned OFF (CH10 low: %d)", ch10_raw);
                }
                prev_ch10_state = current_ch10_state;  // 更新上一次状态
                ch10_stable_count = 0;  // 重置计数
            }
        }
    }
...
int main(int argc, char**argv) {
    ...
    ROS_INFO("LED initialized on wPi pin %d", LED_PIN);

    // 小车参数配置
    pnh.param<float>("linear_x", linear_x, 0.33f);
    pnh.param<float>("turn_speed", turn_speed, 0.5f);
    pnh.param<float>("target_distance", target_distance, 0.1f);
    ...
    init_cmd.data = "0,90";  // 水平不动(0)，垂直朝上(100)
